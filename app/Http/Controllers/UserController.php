...
class UserController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        $drs = DB::table('Bagences')->select('Nom_DR')->groupBy('Nom_DR')->get();
        ...
        $utilisateurs = User::leftJoin('Bagences', function ($join) {
            $join->on('Bagences.code_ag', '=', 'Busers.Structure');
        })
        ...
        if ($privilege->visibilite == "N") {
            if ($request->dr_id) {
                $utilisateurs = $utilisateurs->where("Busers.Nom_DR", $request->dr_id);
            }
            if ($request->str_id) {
            ...
            $utilisateurs = $utilisateurs->where("Busers.Structure", Auth::user()->Structure);

        } elseif ($privilege->visibilite == "P") {
            $utilisateurs = $utilisateurs->where('Busers.Matricule','=', Auth::user()->Matricule) ;
        }
        if ($request->actif=='0') {
        ...
        $utilisateurs =  $utilisateurs->select('Matricule', 'Nom', 'Prénom', 'Busers.Nom_DR', 'code_ag', 'nom_ag', 'privilege', 'Fonction', 'Busers.email', 'Structure', 'LibelleFct', 'Actif', 'Date_activation')->get();
        $fonctions = DB::table('Bfonction')->orderBy('LibelleFct', 'asc')->get();

        // return response()->json(['utilisateurs' => $utilisateurs], 200);
        return response()->json(['utilisateurs' => $utilisateurs, 'profils' => $profils, 'drs' => $drs, 'fonctions' => $fonctions, 'str_id' => $request->str_id, 'dr_id' => $request->dr_id], 200);
    }
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'Matricule' => 'required|max:5|min:5|unique:Busers,Matricule',
            'Nom' => 'required',
            'Prénom' => 'required',
            'Nom_DR' => 'required',
            'Structure' => 'required',
        ...
        $data['Nom_DR'] = $request->Nom_DR;
        $data['Structure'] = $request->Structure;
        $data['Fonction'] = $request->Fonction;
        $data['email'] = $request->email;
        $data['passwordPlatforme'] = Hash::make($request->password);
        DB::table('Busers')->insert($data);
    ...
    public function update(Request $request, $matricule)
    {
        ...
        // return response()->json(['matricule' =>$matricule,'request' =>$request->all(),],525);
        ...
        DB::table('Busers')->where('Matricule', $matricule)->update($data);
    ...
    {
        DB::statement("UPDATE Busers SET privilege = null WHERE Matricule = '" . $Matricule . "'");
