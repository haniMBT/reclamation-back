...
*/
	...
	function Mille($nombre, $unite, $dizaine, $centaine, $mille)
	{
		...
		$nb = strlen($cent);
		#echo '<br />nb='.$nb.'<br />';
		if ($nb == 0); //AffichageResultat($enLettre);
		elseif ($nb == 1)
			$this->Unite($cent);
		elseif ($nb == 2)
			$this->Dizaine(0, $cent, $unite, $dizaine);
	...
	{
		if (!empty($unite) && array_key_exists($unite, $this->chiffre)) {
	...
	function get_conversion($nombre)
	{
		...
		if ($pos) {
			...
			$chaine_retour = $this->Conversion($entier) . $chaine_euro . ' et ' . $this->Conversion($decimale) . $chaine_cts;
		} else {

			$chaine_euro = 'dinar';
			if ($nombre > 1) $chaine_euro .= 's';
			$chaine_retour = $this->Conversion($nombre) . $chaine_euro;
		}
